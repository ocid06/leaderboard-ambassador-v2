...
interface Props {
  ambassadorData: AmbassadorDb[];
}
export default function HomeTemplates({ ambassadorData }: Props) {
  const [ambassadors, setAmbassadors] =
    useState<AmbassadorDb[]>(ambassadorData);
  ...
  const stats = useMemo(() => {
    ...
    const totalInvites = ambassadors.reduce(
      (sum, amb) => sum + (amb.invites || 0),
      0
    );
    const avgScore = Math.round(
      ambassadors.reduce((sum, amb) => sum + (amb.score || 0), 0) /
        ambassadors.length
    );
  ...
  return (
    <main className="min-h-screen bg-linear-to-br from-black via-black to-black">
      {/* Background Effects */}
            ...
            <div className="flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-gray-400 mt-1">
                  Ambassador Rankings & Performance
                </p>
              </div>
            </div>
          </div>
